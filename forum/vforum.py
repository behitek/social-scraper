...
def is_post(url):
    return re.match(r'https?://vforum\.vn/diendan/showthread\.php.+', url) is not None
...
def correct_url(url):
    ...
    if not url.startswith('http'):
        url = "https://vforum.vn/diendan/" + url
    url = re.sub('#.+$', '', url)
    url = re.sub('/*$', '/', url)
    if re.match(r'https?://vforum\.vn/diendan/.+', url):
        return url
    return ''


home_url = "https://vforum.vn/diendan/index.php"
...
def get_comment(url):
    ...
    with open('vforum.txt', 'a+', encoding='utf8') as fp:
